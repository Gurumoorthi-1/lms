         ...
         {exam.status === 'disqualified' ? (
           ...
           <div className="text-right">
             ...
             <div className="text-[10px] uppercase font-bold text-[#64748B]">
               {exam.score >= 70 && progress?.status !== 'COMPLETED' ? 'In Progress' : (exam.score > 0 && exam.score < 70 ? 'Retake Required' : 'Ready')}
             </div>
           </div>
         )}
...
export default function StudentDashboard() {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const res = await authFetch(`/exams/${examToDelete}`, { method: 'DELETE' });
      if (res.ok) {
        setRecentExams(prev => prev.filter(e => e._id !== examToDelete));
        toast.success("Assessment deleted successfully.", { icon: '🗑️' });
      } else {
  ...
  return (
    <DashboardLayout>
      ...
      <Modal 
        ...
        onClose={() => { setDeleteModalOpen(false); setExamToDelete(null); }}
        showClose={true}
        className="bg-white"
        maxWidth="max-w-md"
      >
        <div className="text-center space-y-6">
          ...
          <div className="space-y-2">
            <h3 className="text-2xl font-black text-[#0F172A] tracking-tight">Delete Assessment?</h3>
            <p className="text-[#64748B] text-sm font-medium leading-relaxed px-2">
          ...
          <div className="flex gap-3 pt-2">
            <button
              ...
              className="flex-1 h-12 bg-[#F8FAFC] hover:bg-[#F1F5F9] text-[#64748B] font-bold rounded-2xl transition-all"
            >
              Cancel
            </button>
            <button
              onClick={handleConfirmDelete}
              className="flex-1 h-12 bg-red-600 hover:bg-red-500 text-white font-black rounded-2xl transition-all shadow-lg shadow-red-100"
            >
              Delete
            </button>
          </div>
