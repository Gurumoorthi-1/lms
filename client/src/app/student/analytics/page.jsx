...
export default function AnalyticsPage() {
  ...
  // Derived Metrics
  const isInstitutional = !!performanceProfile?.isInstitutional || !!performanceProfile?.institutionId;
  const mcqScore = performanceProfile?.mcq?.score || 0;
  ...
  const overallScore = performanceProfile?.overallScore !== undefined 
      ? performanceProfile.overallScore 
      : (isInstitutional
          ? Math.round(((aptScore * 0.2) + (codingScore * 0.3) + (hrPercentage * 0.3)) / 0.8)
          : Math.round((mcqScore * 0.2) + (aptScore * 0.2) + (codingScore * 0.3) + (hrPercentage * 0.3)));
        ...
        {isDataLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
              ...
              {(aptitudeResults || performanceProfile?.aptitude?.percentage !== undefined) ? (
                <p className="text-sm font-medium text-gray-500">
                  Correctly answered <span className="text-gray-800 font-bold">{aptitudeResults ? aptitudeResults.score : Math.round(((performanceProfile?.aptitude?.percentage || 0) / 100) * 10)}</span> out of {aptitudeResults ? aptitudeResults.maxScore : 10} questions.
                </p>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
              ...
              {(codingResults || performanceProfile?.coding?.marksObtained !== undefined) ? (
                <p className="text-sm font-medium text-gray-500">
                  Successfully completed <span className="text-gray-800 font-bold">{Object.keys(codingTasks).length || performanceProfile?.coding?.passedCount || 0}</span> tasks.
                </p>
              ) : <p className="text-sm text-gray-400">Round skipped or incomplete.</p>}
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
              ...
              {(interviewResults || performanceProfile?.hr?.marksObtained !== undefined) ? (
                <p className="text-sm font-medium text-gray-500">
                  Answered {hrAnalyses.length || (performanceProfile?.hr?.responses ? Object.keys(performanceProfile?.hr?.responses).length : 7)} out of 7 questions.
                </p>
              ) : <p className="text-sm text-gray-400">Round skipped or incomplete.</p>}
            </motion.div>
          </div>
