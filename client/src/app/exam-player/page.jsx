...
import {
  ...
  Shield, ShieldAlert, PlayCircle, Lock, ArrowRight, ShieldCheck, Camera, User
} from 'lucide-react';
...
export default function ExamPlayer() {
  ...
  const handleRetest = useCallback(() => {
    ...
    setCameraViolations(0); setTabViolations(0); setFsViolations(0); setScreenshotViolations(0); setIsStarted(true);
  }, [totalTime]);
  ...
  const handleViolation = useCallback(async (reason, severity = 'warning', type = 'general') => {
    ...
    if (isFs) {
      ...
      setScreenshotViolations(prev => {
        const newCount = prev + 1;
        toast.error(`Security Alert: ${reason}. Screenshot attempts: ${newCount}/1`, { 
          id: 'security_alert_screenshot', 
          duration: 5000, 
          icon: <ShieldAlert size={20} className="text-red-500" />, 
          style: { border: '2px solid #ef4444', background: '#0f172a', color: '#fff' } 
        });
  ...
  useEffect(() => {
    if (cameraViolations >= 4 || tabViolations >= 2 || fsViolations >= 2 || screenshotViolations >= 1) {
  ...
  }, [cameraViolations, tabViolations, fsViolations, screenshotViolations, isSubmitting, showSuccess, handleFinish]);
      ...
      if (isWinShiftS || isMacCmdShift || isPrtScnKey) {
        ...
        try {
          if (navigator.clipboard && navigator.clipboard.writeText) {
            navigator.clipboard.writeText('🚫 Screenshots and copying are strictly prohibited during this exam! Your attempt has been logged.');
          }
        } catch (err) {}
        handleViolation("Screenshot attempt detected", "severe", "screenshot");
      }
    };
  ...
  if (!validExam) {
    return (
      <div className="h-screen bg-[#0F172A] flex flex-col items-center justify-center p-6 text-center select-none">
        ...
        <h1 className="text-3xl font-black mb-2 text-white tracking-tight">No Interview Found</h1>
        <p className="text-slate-400 font-medium max-w-md mb-8">
          You haven't been assigned an interview or you have deleted your recent assessment. Please return to the dashboard to generate a new AI assessment.
        </p>
        <Link to="/student">
          <button className="h-14 px-8 bg-[#2563EB] hover:bg-[#1D4ED8] transition-all text-white rounded-2xl font-black shadow-lg shadow-blue-900/40 flex items-center gap-2 mx-auto">
          ...
          <div className="grid grid-cols-2 gap-4 text-left">
             ...
             <div className="bg-white/5 p-4 rounded-2xl border border-white/10">
                ...
                <div className="text-xs font-bold text-white leading-relaxed">
                  ...
                  • FS Exit: 2 attempts<br/>
                  • Screenshot: 1 attempt
                </div>
             </div>
          </div>
      ...
      <header className="h-16 bg-white border-b border-[#E2E8F0] flex items-center justify-between px-6 shrink-0">
        ...
        <div className="flex items-center gap-3">
          <div className="hidden md:flex items-center gap-4 mr-4 px-4 py-2 bg-slate-50 border border-[#E2E8F0] rounded-xl text-xs font-black">
             ...
             <div className="w-px h-3 bg-[#E2E8F0]" />
             <div className="flex items-center gap-2 text-[#64748B] uppercase tracking-tighter">Screenshots: <span className={screenshotViolations > 0 ? 'text-red-500' : ''}>{screenshotViolations}/1</span></div>
          </div>
