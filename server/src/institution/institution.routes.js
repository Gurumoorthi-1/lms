    ...
    .on('end', async () => {
      try {
        ...
        for (const row of results) {
          ...
          const username = row.username || row.Username || row.USERNAME || row.name || normalizedEmail.split('@')[0];
...
router.get('/users', requireAuth, async (req, res) => {
  try {
    const users = await User.find({ createdBy: req.user.userId }).select('-password');
    res.json(users);
...
router.delete('/users', requireAuth, async (req, res) => {
  try {
    await User.deleteMany({ createdBy: req.user.userId });
    res.json({ success: true, message: 'All institutional users deleted.' });
  } catch (error) { res.status(400).json({ message: error.message }); }
});
